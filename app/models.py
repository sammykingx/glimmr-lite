...
class User(db.Model):
    ...
    booked_service = db.relationship(
        "Booking",
        back_populates="user",
        lazy=True,
    )
...
class Booking(db.Model):
    ...
    booking_time = db.Column(
        db.Time, default=datetime.now(TORONRO_TZ).time(), nullable=False
    )
...
class RecurringBooking(db.Model):
    ...
    booking = db.relationship(
        "Booking", backref=db.backref("recurring_bookings", lazy=True)
    )
